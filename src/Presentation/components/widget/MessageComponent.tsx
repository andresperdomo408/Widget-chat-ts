  ...
  const MessageComponent: React.FC<MessageProps> = ({ message,buttonResponse }) => {
      ...
      <div key={message.name} className={`message ${messageClass}`}>
        <div style={{ maxWidth: "100%", wordWrap: "break-word", marginBottom: "10px" }}>
          ...
          >
            ...
            {message.text && <TextComponent text={message.text}  date={message.createdAt!}/>}
            {message.nodes && message.nodes.length > 0 && (
            <NodesComponent nodes={message.nodes} buttonResponse={buttonResponse} />
      )}
          </div>
        </div>
      </div>
